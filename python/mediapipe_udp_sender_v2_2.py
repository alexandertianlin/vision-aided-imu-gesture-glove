...
def compute_finger_angles(points, finger_name):
    """Extract per-joint bend angles from MediaPipe 3D world landmarks."""
    finger = FINGERS[finger_name]
    if finger_name == "Thumb":
        b1 = sub(points[finger["pip"]], points[finger["mcp"]])
        b2 = sub(points[finger["tip"]], points[finger["pip"]])
    else:
        mcp = points[finger["mcp"]]; pip = points[finger["pip"]]
        dip = points[finger["dip"]]; tip = points[finger["tip"]]
        b1 = sub(pip, mcp); b2 = sub(dip, pip); b3 = sub(tip, dip)

    def va(a, b):
        d = a[0]*b[0]+a[1]*b[1]+a[2]*b[2]
        return math.degrees(math.acos(max(-1,min(1,d/(max(1e-8,norm(a))*max(1e-8,norm(b)))))))

    if finger_name == "Thumb":
        total = va(b1, b2); joints = [total, 0.0]
    else:
        total = va(b1, b2) + va(b2, b3); joints = [va(b1, b2), va(b2, b3)]

    wrist = points[0]; imcp = points[5]; lmcp = points[17]
    fdir = sub(points[finger["tip"]], points[finger["mcp"]])
    palm_n = cross(sub(imcp, wrist), sub(lmcp, wrist))
    yaw = va(fdir, palm_n) - 90.0

    return total, max(-90.0,min(90.0,yaw)), joints


def send_angle_packet(sock, fi, pitch, yaw, joints, conf, seq):
    """Send continuous per-finger angle packet."""
    p = {"ts": int(time.monotonic()*1000), "seq": seq, "type": "ang",
         "fi": fi, "p": round(float(pitch),1), "y": round(float(yaw),1),
         "ja": [round(float(a),1) for a in joints], "c": round(float(conf),3)}
    data = json.dumps(p, separators=(",",":")).encode("utf-8")
    sock.sendto(data, (UNITY_IP, UNITY_PORT))
    print(f"ANGLE seq={seq} fi={fi} p={p["p"]} y={p["y"]}")
...
def main():
    ...
    print(f"[V2.2] D435i continuous finger angle sender -> Unity UDP {UNITY_IP}:{UNITY_PORT}")
    ...
    with hand_landmarker.create_from_options(options) as landmarker:
        while cap.isOpened():
            ...
            cv2.imshow("V2.2 Continuous Angle Sender", frame)
